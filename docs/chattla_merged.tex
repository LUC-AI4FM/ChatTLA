...
\bottomrule
\end{tabular}
\caption{20-problem benchmark trajectory across checkpoints.
``Piecewise'' shares weights with \texttt{v13}$_R$ and differs only
in the inference procedure.  Counts read from the underlying CSVs in
\texttt{outputs/benchmark\_results/}.}
...
M.~Ortiz, ``FormaLLM: TLA\textsuperscript{+} benchmark and evaluation
suite for large language models,'' Technical report, 2026.
